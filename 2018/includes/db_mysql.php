...
function db_connect( $hostname, $dbname,$user, $password, $dbport='3306', $persist=false ) {
/*function db_connect( $dbhost='127.0.0.1', $dbname='hosxp_pcu', $dbuser='sa', $dbpas='sa', $dbport='3306', $persist=false ) {*/
	function_exists( 'mysql_connect' )
		or  die( 'FATAL ERROR: MySQL support not avaiable.  Please check your configuration.' );

	if ($persist) {
		mysql_pconnect( "$hostname:$dbport", $user, $password )
			or die( 'FATAL ERROR: Connection to database server failed' );
	} else {
		mysql_connect( "$hostname:$dbport", $user, $password )
			or die( 'FATAL ERROR: Connection to database server failed' );
	}
